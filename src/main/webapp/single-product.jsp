...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="col-lg-9">
            ...
            <div class="card mt-4">
                <img class="card-img-top img-fluid" src="img/product.jpg" alt="">
                <div class="card-body">
                    <h3 class="card-title">${product.name}</h3>
                    <h4>$ ${product.price}</h4>
                    <p class="card-text">${product.type}</p>
                    <p class="card-text">${product.caliber}</p>
                    <p class="card-text">${product.color}</p>
                    <p class="card-text">${product.producer}</p>
                    <form action = "/product?product_id=${product.id}"  method = "POST">
                            <input name="addToCart" value="${product.id}">
                        <input type = "submit"  value = "Submit" />
                    </form>
                    <a type="button" href="/product?product_id=${product.id}&addToCart=${product.id}" class="btn btn-outline-primary">Add to Cart</a>
                    <a type="button" class="btn btn-outline-warning">Buy</a>
                </div>
                <!-- /.card -->
